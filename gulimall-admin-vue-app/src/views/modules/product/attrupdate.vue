...
export default {
  ...
  methods: {
    ...
    getSpuBaseAttrs() {
      this.$http({
        url: this.$http.adornUrl(`/provider/attr/base/listforspu/${this.spuId}`),
        method: "get"
      }).then(({ data }) => {
    ...
    showBaseAttrs() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          `/provider/attrgroup/${this.catalogId}/withattr`
        ),
        method: "get",
    ...
    submitSpuAttrs() {
      ...
      this.$confirm("修改商品规格信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(`/provider/attr/update/${this.spuId}`),
            method: "post",
